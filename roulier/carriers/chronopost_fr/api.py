...
class ChronopostFrApi(Api):
    def _service(self):
        schema = super(ChronopostFrApi, self)._service()
    ...
    def _parcel(self):
        schema = super(ChronopostFrApi, self)._parcel()
    ...
    def _address(self):
        schema = super(ChronopostFrApi, self)._address()
        additional_fields = {
            "civility": {
                "required": True,
                "empty": False,
    ...
    def _to_address(self):
        schema = super(ChronopostFrApi, self)._to_address()
        schema["preAlert"].update({"allowed": [0, 22]})
        return schema

    def _from_address(self):
        schema = super(ChronopostFrApi, self)._from_address()
        schema["preAlert"].update({"allowed": [0, 11]})
        return schema

    def _auth(self):
        schema = super(ChronopostFrApi, self)._auth()
